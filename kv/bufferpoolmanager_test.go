...
func TestBufferPoolManager(t *testing.T) {
	...
	bpm.GetNewNode() // id=4
	AssertEqual(t, (*bpm.pool[2]).getID(), NodeID(4))
	...
	bpm.FetchNode(1)
	bpm.FetchNode(2)
	AssertEqual(t, (*bpm.pool[1]).getID(), NodeID(1)) //put into frame 2 and 3, since we unpinned 6,7 wich was in there
	AssertEqual(t, (*bpm.pool[2]).getID(), NodeID(2))
